...
import User from "../modals/user.modal"; // your mongoose model

const JWT_SECRET = process.env.JWT_SECRET || "secret";

export const signupController = async ({ body }: { body: { name: string; email: string; password: string } }) => {
  const { name, email, password } = body;

  const existingUser = await User.findOne({ email });
  if (existingUser) return { status: 400, body: { message: "User already exists" } };
...
export const signinController = async ({ body }: { body: { email: string; password: string } }) => {
